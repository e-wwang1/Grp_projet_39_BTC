...
Pour conclure, ce projet permet de manipuler des données expérimentales pour générer des graphiques et de mieux comprendre leur évolution grâce à ces graphiques. Même si le programme reste basique et présente certaines limites, il constitue cependant une bonne base pour analyser des données biologiques. En effet, dans cette étude, nous avons pu analyser l'impact d'un traitement antibiotique sur le microbiote intestinal chez la souris. Le programme aurait pu être plus optimal car plusieurs lignes se répétaient ainsi cela faisait des ralonges dans le code. Ce projet nous a permis de mettre en pratique nos connaissances en informatique et de l'employer dans la biologie. Il nous a également appris à faire des recherches pour trouver des codes notamment pour la mise en forme des graphiques et transformer des données des listes en un fichier CSV et de l'ajouter dans un dossier. 
